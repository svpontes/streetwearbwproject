...
namespace StreetTshirtApp.Services
{
    public class CartService
    {
        ...
        {
            var existingItem = _cartItems.FirstOrDefault(i => i.Product.Id == product.Id && i.Size == size);
            ...
            {
                _cartItems.Add(new CartItem { Product = product, Size = size, Quantity = quantity });
            }
        ...
        {
            var item = _cartItems.FirstOrDefault(i => i.Product.Id == productId && i.Size == size);
            if (item != null)
            {
    ...
    // ESTA CLASSE PRECISA ESTAR AQUI (FORA DA CARTSERVICE)
